...
const sendResetPasswordEmail = async (to, token) => {
  ...
  const resetPasswordUrl = `http://${config.backend.auth}/reset-password?token=${token}`;
...
const sendVerificationEmail = async (to, token) => {
  ...
  const verificationEmailUrl = `http://${config.backend.auth}/v1/auth/verify-email?token=${token}`;
  const text = `Dear user,
...
http://${config.backend.auth}/dashboard/admin/detail?bearId=${bearId}`;
  await sendEmail(to, subject, text);
};
